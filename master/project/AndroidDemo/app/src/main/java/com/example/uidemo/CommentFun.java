...
//                textView = (TextView) View.inflate(context, R.layout.trends_comment_list_item, null);
//                commentList.addView(textView);
//            }
//            textView.setVisibility(View.VISIBLE);
//            if (comment.getmReceiver() == null) { // 没有评论接受者
//                content = String.format("<html><%s>%s</%s>: <%s>%s</%s></html>", CustomTagHandler.TAG_COMMENTATOR,
//                        comment.getmCommentator().getUsername(), CustomTagHandler.TAG_COMMENTATOR,
//                        CustomTagHandler.TAG_CONTENT, comment.getmContent(), CustomTagHandler.TAG_CONTENT);
//            } else {
//                content = String.format("<html><%s>%s</%s> 回复 <%s>%s</%s>: <%s>%s</%s><html>",
//                        CustomTagHandler.TAG_COMMENTATOR, comment.getmCommentator().getUsername(), CustomTagHandler.TAG_COMMENTATOR,
//                        CustomTagHandler.TAG_RECEIVER, comment.getmReceiver().getUsername(), CustomTagHandler.TAG_RECEIVER,
//                        CustomTagHandler.TAG_CONTENT, comment.getmContent(), CustomTagHandler.TAG_CONTENT);
//            }
//            textView.setText(Html.fromHtml(content, null, tagHandler)); // 解析标签
//            textView.setClickable(true);
//            textView.setMovementMethod(LinkMovementMethod.getInstance());
//            textView.setTag(CustomTagHandler.KEY_COMMENTATOR, comment.getmCommentator());
//            textView.setTag(CustomTagHandler.KEY_RECEIVER, comment.getmReceiver());
//
...
//        String hint;
//        if (receiver != null) {
//            if (receiver.getUserId() == MainActivity.currentUser.getUserId()) {
//                hint = "我也说一句";
//            } else {
//                hint = "回复 " + receiver.getUsername();
//            }
//        } else {
//            hint = "我也说一句";
//        }
...
//                final long receiverId = receiver == null ? -1 : receiver.getUserId();
//                Comment comment = new Comment(MainActivity.currentUser, content, receiver);
