...
public class SitUpTimingActivity extends AppCompatActivity {
    ...
    private void initVideo() {
        ...
        playerStandard.setUp(ConfigUtil.SERVER_ADDR + "/video/yifenzhongyangwoqizuo.mp4", JCVideoPlayerStandard.SCREEN_LAYOUT_NORMAL, "一分钟仰卧起坐");
    ...
    private void nextItem(JsonObject jo) {
        ...
        jsonObject.addProperty("yfzywqz", tvData.getText().toString());
        if (jo.get("nj").getAsString().equals("3") || jo.get("nj").getAsString().equals("4")){
            intent.putExtra("json",jo.toString());
            intent.setClass(SitUpTimingActivity.this,ResultActivity.class);
        }else {
            intent.putExtra("json",jo.toString());
            intent.setClass(SitUpTimingActivity.this,FiftymeterRunBackAcitivity.class);
        }
